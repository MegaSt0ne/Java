...
public class Homework1_3 {
	...
	public static void main(String[] args) {
		...
		double monthlyPayment = ( interestRate * ( principal / PAYMENT_PER_YEAR )) 
				/ ( 1 - Math.pow( ( ( interestRate / PAYMENT_PER_YEAR ) + 1 ), 
						( -1 * PAYMENT_PER_YEAR * years) ) );
		...
		for ( int i = 1; i <= years ; i++ ) {
			System.out.printf("The total payment after %d year: %,.3f \n", i, monthlyPayment * 12 * i);
		}
		
		//output total interest paid during the course of the loan
		System.out.printf("The total interest paid during the course of the loan is: %,.3f \n", monthlyPayment * 12 * years - principal);
	}
